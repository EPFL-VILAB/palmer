...
class SimpleExplorationGameStochastic(GameTemplate):
    '''
    ...
    '''
    def __init__(self, cfg_path, max_drift=3):
    ...
    def implement_action(self, action):
        ...
        drift = self.rng.integers(1, self.max_drift+1)
        random_action = self.rng.integers(0, len(self.buttons))
        
        self.game.set_action(self.buttons[random_action])
        self.game.advance_action(drift, False)
    ...
    def map_to_pixel(self, point, res_x=480):
    ...
    def draw_vis_map(self, res_x=480):
        ...
        for endpoints in self.line_endpoints:
            self.vis_map = cv2.line(self.vis_map, self.map_to_pixel(endpoints[0]), self.map_to_pixel(endpoints[1]), (0, 0, 0), thickness=2)
    ...
    def draw_points(self, points, colors=None):
        ...
        '''
        if colors is None: 
            colors = [(0, 0, 255) for i in range(len(points))]
            
        for point, color in zip(points, colors):
            self.vis_map = cv2.circle(self.vis_map, self.map_to_pixel(point), 4, color, thickness=-1)
            
    def draw_trajectory(self, points, colors=None):
        '''
        Draws points on the current map.
        - points: shape (N, >2)
        - colors: shape (N, 3)
        ...
        for i in range(1, len(points)):
            self.vis_map = cv2.line(self.vis_map, self.map_to_pixel(points[i-1]), self.map_to_pixel(points[i]), (0, 0, 0), thickness=1)
            
        for i in range(1, len(points)):
            self.vis_map = cv2.circle(self.vis_map, self.map_to_pixel(points[i]), 4, colors[i], thickness=-1)
            
        self.vis_map = cv2.circle(self.vis_map, self.map_to_pixel(points[0]), 4, (255, 0, 0), thickness=-1)
        self.vis_map = cv2.circle(self.vis_map, self.map_to_pixel(points[-1]), 4, (0, 255, 0), thickness=-1)
    ...
    def draw_graph(self, positions, pairwise_distances, edge_distance_threshold):
        ...
        for i in range(num_samples):
            self.vis_map = cv2.circle(self.vis_map, self.map_to_pixel(positions[i]), 4, (0, 0, 255), thickness=-1)
            
        for i in range(num_samples):
            for j in range(num_samples):
                pT_distance = pairwise_distances[i, j]
                if pT_distance < edge_distance_threshold:
                    self.vis_map = cv2.line(self.vis_map, self.map_to_pixel(positions[i]), self.map_to_pixel(positions[j]), (3, 44, 46), thickness=1)
