...
class GraphMemory():
    ...
    def steer_nearest_node_to_point(self, nearest_node_tree_id, nearest_node_walk_id):
        ...
        if overlap_flag and self.overlap_node_tree_id is None:
            self.overlap_node_tree_id = len(self.tree_node_walk_ids)
        
        if not overlap_flag:
            new_point_walk_id = retrieval_trajectory_steer_segment[-1]
            new_point_position = self.positions_block[new_point_walk_id]
            new_point_latent = self.latents_block[new_point_walk_id]
            line_cost = cut_off # the first node in the retrieved trajectory doesn't count as a step?
            self.latent_distances_new_point_to_walk, self.latent_distance_masks_new_point_to_walk =  self.compute_latent_distances_to_walk(new_point_latent)
        ...
        self.tree_node_walk_ids.append(new_point_walk_id)
    ...
    def render_RRT_graph(self, idx=0):
        ...
        Image.fromarray(self.env.game.vis_map).save('RRT_star_graph_{}.png'.format(idx))
    ...
    def render_RRT_trajectory(self, idx=0):
        ...
        figure(figsize=(10, 10))
        plt.imshow(self.env.game.vis_map)
        Image.fromarray(self.env.game.vis_map).save('RRT_star_path_{}.png'.format(idx))
    ...
    def render_stitched_path(self, path, original_path=None, idx=0):
        ...
        if original_path is not None:
            for point_id in original_path[1:-1]:
                self.env.game.draw_point(self.tree_node_positions[point_id])
                
        figure(figsize=(10, 10))
        plt.imshow(self.env.game.vis_map)
        Image.fromarray(self.env.game.vis_map).save('RRT_star_stitched_path_{}.png'.format(idx))
