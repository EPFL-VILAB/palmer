...
class GraphMemory():
    ...
    def steer_nearest_node_to_point(self, nearest_node_tree_id, nearest_node_walk_id):
        ...
        if not overlap_flag:
            new_point_walk_id = retrieval_trajectory_steer_segment[-1]
            new_point_position = self.positions_block[new_point_walk_id]
            new_point_latent = self.latents_block[new_point_walk_id]
            self.latent_distances_new_point_to_walk, self.latent_distance_masks_new_point_to_walk =  self.compute_latent_distances_to_walk(new_point_latent)
        ...
        # TODO: When retrieving actions, do additional retrievals on the RRT graph
        
        return overlap_flag, retrieval_distance_new_point_to_goal
    
    #-----------------------------------------------------------------------------------------------
    # Build RRT
    #-----------------------------------------------------------------------------------------------
    
    def build_RRT(self, num_iterations, root_latent, root_position, goal_latent, goal_position, iter_lower_bound=0):
        ...
        for iter_num in tqdm(range(num_iterations)):
            try:
                ...
                overlap_flag, overlap_retrieval_distance = self.steer_nearest_node_to_point(nearest_node_tree_id, nearest_node_walk_id)
                self.overlap_retrieval_distances.append(overlap_retrieval_distance)
                if overlap_flag: 
                    if iter_lower_bound <= iter_num:
                        break
            except:
                pass
    ...
    def flush_graph(self):
        ...
        self.latent_distances_walk_to_point = None # shape (num_new_samples, num_samples_random_walk)
        self.latent_distance_masks_walk_to_point = None # shape (num_new_samples, num_samples_random_walk)
        self.retrieval_distances_nodes_to_point = None # shape (num_new_samples, num_samples)
    ...
    def render_RRT_graph(self, idx=0):
        ...
        Image.fromarray(self.env.game.vis_map).save('RRT_graph_{}.png'.format(idx))
        
    def render_RRT_graph_animated(self):

        self.env.game.draw_vis_map()
        frames = self.env.game.draw_RRT_graph_animated(self.goal_position, self.tree_node_positions, self.tree_node_parent_tree_ids, self.random_point_positions, self.retrieval_trajectory_positions)
        return frames
    ...
    def get_RRT_trajectory(self):
        ...
        RRT_trajectory_node_tree_ids = []
        RRT_trajectory_walk_ids = [self.tree_node_walk_ids[-1]] + RRT_trajectory_walk_ids
        RRT_trajectory_positions = [self.tree_node_positions[-1]] + RRT_trajectory_positions
        RRT_trajectory_node_tree_ids = [len(self.tree_node_walk_ids) - 1]
        parent_id = self.tree_node_parent_tree_ids[-1]
        ...
        while parent_id != 0.5:
            ...
            RRT_trajectory_node_tree_ids = [parent_id] + RRT_trajectory_node_tree_ids
            parent_id = self.tree_node_parent_tree_ids[parent_id]
            
        RRT_trajectory_walk_ids[0] = 'start'
        RRT_trajectory_walk_ids.append('goal')
        RRT_trajectory_node_tree_ids.append('goal')
        RRT_trajectory_positions = RRT_trajectory_positions + [self.goal_position]

        return RRT_trajectory_node_tree_ids, RRT_trajectory_walk_ids, RRT_trajectory_positions
    
    def render_RRT_trajectory(self, idx=0):
        
        _, _, RRT_trajectory_positions = self.get_RRT_trajectory()
        self.env.game.draw_vis_map()
        self.env.game.draw_trajectory(RRT_trajectory_positions)
        figure(figsize=(10, 10))
        plt.imshow(self.env.game.vis_map)
        Image.fromarray(self.env.game.vis_map).save('RRT_path_{}.png'.format(idx))
        
    def get_retrieval_trajectory(self, sample_id_current, sample_id_target):
        
        mask_current = self.latent_distance_masks_nodes_to_walk[sample_id_current]
        indices_current = np.array(mask_current.nonzero())
        
        if sample_id_target == 'goal':
            mask_target = self.latent_distance_masks_walk_to_goal.squeeze()
        else:
            mask_target = self.latent_distance_masks_nodes_to_walk[sample_id_target]
    ...
    def stitch_path(self, path):
        '''
        - path is a sequence of tree ids
        ...
        for path_segment in zip(path[:-1], path[1:]):
            ...
            if path_segment[0] == 0:
                ...
                stitched_path.append(self.tree_node_walk_ids[path_segment[0]])
        ...
        if path_segment[1] == 'goal':
            ...
            stitched_path.append(self.tree_node_walk_ids[path_segment[1]])

        return stitched_path
    
    def render_stitched_path(self, path, original_positions=None, idx=0):
        
        if path[0] == 'start':
            path_positions = [self.tree_node_positions[0]] + self.positions_block[path[1:-1]].tolist() + [self.goal_position]
            
        else:
            path_positions = self.positions_block[path]
        
        self.env.game.draw_vis_map()
        self.env.game.draw_trajectory(path_positions)
        
        if original_positions is not None:
            for point_position in original_positions[1:-1]:
                self.env.game.draw_point(point_position)
                
        figure(figsize=(10, 10))
        plt.imshow(self.env.game.vis_map)
        Image.fromarray(self.env.game.vis_map).save('RRT_stitched_path_{}.png'.format(idx))
